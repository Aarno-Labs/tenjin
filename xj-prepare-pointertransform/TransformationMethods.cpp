...
bool FunctionAccessAnalyzer::generateTransformation(
    ...
    if (!candidate.is_parameter) {
        ...
        // Two shapes cannot put the index after the DeclStmt, and both take
        // the anchor path, which places it before the nearest position that
        // accepts a statement.
        //
        // Handle mode never replaces the declaration — the pointer stays as
        // the handle — and its index initializer is the literal 0, so it has
        // no dependencies and can always make that move. A collapsed
        // multi-declarator for-init has nowhere after the statement to go,
